...
class PluginsForm(forms.Form):
    plugins = forms.ModelMultipleChoiceField(
        required=False, queryset=models.Plugin.objects.all(), widget=forms.CheckboxSelectMultiple()
    )
    ...
    def save(self):
        ...
        for plugin in self.cleaned_data["plugins"]:
            models.ActivePlugin.objects.create(plugin=plugin, channel=self.channel)
