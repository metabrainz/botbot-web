...
class ChannelMixin:
    ...
    def get_channel(self, user, **kwargs):
        ...
        if not self._channel:
            ...
            if channel_pk:
                ...
                channel = get_object_or_404(self._channel_queryset(), private_slug=kwargs["channel_slug"])

            else:
                channel = self._get_identifiable_channel(kwargs["bot_slug"], kwargs["channel_slug"])
    ...
    def _get_identifiable_channel(self, bot_slug, channel_slug):
        ...
        """
        candidates = self._channel_queryset().filter(slug=channel_slug, is_public=True).select_related("chatbot")
        ...
        for channel in candidates:
            if channel.chatbot.slug == bot_slug:
                ...
                raise self.LegacySlugUsage(reverse_lazy(self.request.resolver_match.url_name, kwargs=kwargs))
...
class ChannelList(ListView):
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        return qs.filter(chatbot__slug=self.kwargs["network_slug"], is_public=True).active()
