...
try:
    ...
    LOG.warn("Could not open GEOIP database. Map stream is disabled. " "Try `make geoip-db` to download a copy.")
    GEOIP = None
...
def _send_event_with_id(event_name, data, event_id, ip, channel):
    ...
    requests.post(
        settings.PUSH_STREAM_URL.format(id=channel),
        headers={"Event-Id": event_id, "Event-Type": event_name},
        data=data.encode("utf-8"),
    )
    if GEOIP:
        requests.post(
            settings.PUSH_STREAM_URL.format(id="glob"),
