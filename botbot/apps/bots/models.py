...
from botbot.apps.plugins.models import ActivePlugin
...
class ChatBotManager(models.Manager):
    def get_active_slugs(self):
        return [i[0] for i in self.get_queryset().filter(is_active=True).distinct("slug").values_list("slug")]


class NoAvailableChatBots(Exception):
    """
    Raised we we don't have any chat bots available that can be used on the network.
    """
...
class ChatBot(models.Model):
    ...
    server = models.CharField(max_length=100, help_text="Format: irc.example.net:6697")
    server_password = models.CharField(
        max_length=100, blank=True, null=True, help_text="IRC server password - PASS command. Optional"
    )
    server_identifier = models.CharField(max_length=164)

    nick = models.CharField(max_length=64)
    password = models.CharField(
        max_length=100, blank=True, null=True, help_text="Password to identify with NickServ. Optional."
    )
    real_name = models.CharField(max_length=250, help_text="Usually a URL with information about this bot.")
    ...
    def allocate_bot(cls, slug):
        bots = cls.objects.filter(slug="freenode", is_active=True).annotate(Count("channel")).order_by("channel__count")
...
class Channel(TimeStampedModel):
    ...
    STATUS_CHOICES = ((PENDING, "Pending"), (ACTIVE, "Active"), (ARCHIVED, "Archived"), (BANNED, "Banned"))

    # These are the default plugin slugs.
    DEFAULT_PLUGINS = ["logger", "ping", "last_seen", "help", "bangmotivate"]

    chatbot = models.ForeignKey(ChatBot, null=True, on_delete=models.SET_NULL)
    name = models.CharField(max_length=250, help_text="IRC expects room name: #django")
    slug = models.SlugField()
    private_slug = models.SlugField(unique=True, blank=True, null=True, help_text="Slug used for private rooms")

    password = models.CharField(
        max_length=250, blank=True, null=True, help_text="Password (mode +k) if the channel requires one"
    )
    ...
    plugins = models.ManyToManyField("plugins.Plugin", through="plugins.ActivePlugin")
    ...
    def active_plugin_slugs(self):
        ...
        if not cached_plugins:
            ...
            slug_set = {actv.plugin.slug for actv in plugins}
    ...
    def visible_commands_filter(self):
        ...
        """
        return models.Q(
            command__in=["PRIVMSG", "NICK", "NOTICE", "TOPIC", "ACTION", "SHUTDOWN", "JOIN", "QUIT", "PART", "AWAY"]
        )
    ...
    def get_months_active(self):
        ...
        minmax_dict_key = f"minmax_dict_{self.id}_{current_month}"
    ...
    def current_size(self):
        ...
        """
        try:
            usercount = UserCount.objects.get(channel=self, dt=datetime.date.today())
