...
class BaseTestCase(TestCase):
    def setUp(self):
        self.member = account_models.User.objects.create_user(
            username="dupont éîïè", password="secret", email="dupont@botbot.local"
        )
        self.member.is_superuser = True
        self.member.is_staff = True
        self.member.save()
        self.outsider = account_models.User.objects.create_user(
            username="Marie Thérèse", password="secret", email="m.therese@botbot.local"
        )
        self.chatbot = models.ChatBot.objects.create(server="testserver", nick="botbot", is_active=True)
        self.public_channel = models.Channel.objects.create(
            chatbot=self.chatbot, name="#Test", slug="test", is_public=True, status=models.Channel.ACTIVE
        )
        logs_models.Log.objects.create(
            channel=self.public_channel, command="PRIVMSG", timestamp=pytz.utc.localize(datetime.datetime.now())
        )
        self.private_channel = models.Channel.objects.create(
            chatbot=self.chatbot, name="#test-internal", is_public=False
        )
...
class UrlTests(BaseTestCase):
    def assertFormError(self, response, form, field, error_str):
        ...
        self.assertIn(error_str, response.context_data[form].errors[field])
